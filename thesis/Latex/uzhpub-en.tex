...
\begin{figure}[H]
  ...
  \caption{Illustration of states and the probability distribution to change or stay in a state of a Monte Carlo Markov Chain. In state A there is a 0.3 probability to change into state B and a 0.7 probability to stay in state A.}
  \label{fig:MCMC}
\end{figure}
...
\begin{enumerate}
  ...
  \end{align}
  ,where $I \in R^{dxd}$ is the identity matrix, $\alpha^*$ is the forced acceptance rate and suppose $\{\eta_n\}_{n \geq 1} \subset (0,1]$ is a step size sequence decaying to zero.
\end{enumerate}
...
The differential decay rate for the $B^0$ Meson can be expressed in terms of these variables:
 ...
 \end{align}
 The contribution of $H_{eff}^{(u)}$ has a double Cabibbo supression and is therfore usually dropped. It is kept here since it is sensitive to complex phases of decay amplitudes. The operators $\matchal{O}_{i \leq 6}$ are the same as for general $B \rightarrow X_s l^+l^-$ decays, see equation \ref{eq:P}.
