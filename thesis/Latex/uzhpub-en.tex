...
\begin{figure}[H]
  ...
  \caption{ROC curve of the first fold. One can see that all the classifiers competetive in terms of classifing correctly}
  \label{fig:roc}
\end{figure}
So now lets look at the correlations plots, only the correlations with the mass are shown here. The others can be found in appendix \ref{app:corr}.
\begin{figure}[H]
  \centering
...
\begin{figure}[H]
  ...
  \caption{This plot shows the correltation between the bdt decision of the sk_bdtg classifier and the $B_0$ mass.}
  \label{fig:sk_bdtgB0M}
\end{figure}
...
The two classifers with the least correlation are the sk_bdt and the uBoost classifers. Therfore they were used to do the seperation between signal and background in the $B$ $\rightarrow$ $K^{*}$ $\mu$ $\mu$ data.
